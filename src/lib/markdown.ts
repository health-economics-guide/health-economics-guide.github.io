...
export function parse(markdown: string): Document {
  ...
  const marked = new Marked({
    ...
    renderer: {
       ...
       * and payment models. Narrowing them to a phone would make them
       * unreadable, and letting them set the page width makes everything else
       * unreadable, so the table scrolls inside its own box. `tabindex` makes
       * that scroll region reachable from the keyboard, which WCAG 2.1.1
